...
export const verifyToken = async (req, res, next) => { //next is a function that is called to pass control to the next middleware function in case of an error
    try{
        const token = req.header("Authorization"); // fetch the token from the header
        ...
        if(token.startsWith("Bearer")){ // if the token starts with Bearer then remove the Bearer from the token and get the token. Bearer is a type of token that is used to authenticate the user and it is used in the HTTP header. example of a Bearer token is "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjYx"
            token = token.slice(7, token.length); // remove the Bearer from the token
        }
    ...
    // const authHeader = req.headers.token; // fetch the token from the header
    // if (authHeader) {
    //     const token = authHeader.split(" ")[1]; // split the token from the header
    //     jwt.verify(token, process.env.SECRET_KEY, (err, user) => {
    //         if (err) res.status(403).json("Token is not valid!"); // if the token is not valid, respond with an error
    //         req.user = user; // if the token is valid, set the user in the request
    //         next(); // continue
    //     });
    } catch {
